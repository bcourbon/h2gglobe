...
void Packager::makePlot(RooRealVar *mass, RooRealVar *MH, RooAddPdf *pdf, map<int,RooDataSet*> data, string name){
  ...
  TCanvas *canv = new TCanvas();
	RooPlot *dataPlot = mass->frame(Title(name.c_str()),Range(80,125));
	...
	RooPlot *pdfPlot = mass->frame(Title(name.c_str()),Range(80,125));
  ...
  for (int mh=mhLow_; mh<=mhHigh_; mh++){
    ...
    pdf->plotOn(pdfPlot,Normalization(mass->getBins()/80.*(mass->getMax()-mass->getMin())/60.,RooAbsReal::RelativeExpected));
  }
  pdfPlot->Draw();
