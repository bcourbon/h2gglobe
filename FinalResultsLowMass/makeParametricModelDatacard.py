...
if options.isCutBased:
	...
	if options.isMultiPdf:
		...
		dataFile = 'CMS-HGG_finalBkg.root'
		bkgFile = 'CMS-HGG_finalBkg.root'
		dataWS = 'cms_hgg_workspace'
		bkgWS = 'cms_hgg_workspace'
	if options.isBinnedSignal:
		sigFile = 'CMS-HGG_cic_%dTeV_sig_interpolated.root'%sqrts
		...
		sigFile = 'CMS-HGG_SignalFits.root'
		sigWS = 'wsig_%dTeV'%sqrts
fileDetails = {}
...
def printZpeakSyst():
	print 'Zpeak systematics...'
	for c in range(options.ncats):
		outFile.write('hgg_bkg_%dTeV_cat%d_DCB_mean param \n'%(sqrts,c))
		outFile.write('hgg_bkg_%dTeV_cat%d_DCB_sigma param \n'%(sqrts,c))
		outFile.write('hgg_bkg_%dTeV_cat%d_DCB_nCB1 param \n'%(sqrts,c))
		outFile.write('hgg_bkg_%dTeV_cat%d_DCB_nCB2 param \n'%(sqrts,c))
	outFile.write('\n')
...
def getGlobeLine(proc,cat,name):
	th1f_nom = inFile.Get('th1f_sig_%s_mass_m105_cat%d'%(proc,cat))
	th1f_up  = inFile.Get('th1f_sig_%s_mass_m105_cat%d_%sUp01_sigma'%(proc,cat,name))
	th1f_dn  = inFile.Get('th1f_sig_%s_mass_m105_cat%d_%sDown01_sigma'%(proc,cat,name))
